...
export function EphryaChat() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "ephrya",
      content:
        "Hello. I'm Tilo. Ask me anything about your organization's performance, agents, or goals — or let me help you take the next step.",
      timestamp: new Date(),
    },
  ])
  ...
  const generateEphryaResponse = (userMessage: string): ChatMessage => {
  ...
  const handleSendMessage = async () => {
    ...
    const ephryaResponse = generateEphryaResponse(userMessage.content)
    setMessages((prev) => [...prev, ephryaResponse])
    setIsTyping(false)
  }
  ...
  const handleAction = (action: string) => {
    if (action.startsWith("navigate:")) {
      ...
      // Add a confirmation message from Ephrya
      const confirmationMessage: ChatMessage = {
        id: `ephrya-confirm-${Date.now()}`,
        role: "ephrya",
